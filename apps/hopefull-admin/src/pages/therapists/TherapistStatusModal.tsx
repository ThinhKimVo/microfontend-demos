...
interface TherapistStatusModalProps {
  therapist: Therapist;
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <div className="flex items-center gap-3">
            <div className={cn('p-2 rounded-full', isDeactivate ? 'bg-red-100' : 'bg-green-100')}>
              {isDeactivate ? (
            ...
            <h2 className="text-lg font-semibold text-gray-900">
              {isDeactivate ? 'Deactivate' : 'Activate'} Therapist
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4">
          ...
          {/* Warning */}
          <div className={cn('p-4 rounded-lg flex items-start gap-3', isDeactivate ? 'bg-red-50' : 'bg-green-50')}>
            <AlertTriangle className={cn('h-5 w-5 mt-0.5', isDeactivate ? 'text-red-600' : 'text-green-600')} />
            <div>
              <p className={cn('text-sm font-medium', isDeactivate ? 'text-red-800' : 'text-green-800')}>
                {isDeactivate
                  ? 'Deactivating this therapist will:'
                  : 'Activating this therapist will:'}
              </p>
              <ul className={cn('text-sm mt-2 space-y-1', isDeactivate ? 'text-red-700' : 'text-green-700')}>
                {isDeactivate ? (
                  <>
                  ...
                  <>
                    <li>• Make them visible to users</li>
          ...
          <div>
            ...
            />
            <p className="text-xs text-gray-500 mt-1">These notes are for admin use only and won't be sent to the therapist.</p>
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  {isDeactivate
                    ? 'Notify therapist about account deactivation'
                    : 'Notify therapist about account reactivation'}
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
          ...
          <button
            ...
            disabled={isLoading || (isDeactivate && !selectedReason) || (selectedReason === 'Other (specify below)' && !customReason)}
            className={cn(
              'btn',
              isDeactivate ? 'bg-red-600 text-white hover:bg-red-700' : 'btn-primary'
            )}
          >
            {isLoading ? (
